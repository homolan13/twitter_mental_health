...
def main(batch_size, n_epochs, max_patience):
    ...
    it = 0
    output = {k: [] for k in ['training_loss', 'accuracy', 'precision', 'recall', 'f1']}
    ...
    for epoch in range(n_epochs):
        ...
        for batch in tqdm(train_dataloader):
            ...
            if it%100 == 0:
                ...
                output['accuracy'].append(np.array(batch_accuracy).mean())
                output['precision'].append(np.array(batch_precision).mean())
                output['recall'].append(np.array(batch_recall).mean())
                output['f1'].append(np.array(batch_f1).mean())
                
                print(f'Loss: {output["training_loss"][-1][1]:.3f}')
                print(f'Accuracy: {output["accuracy"][-1]:.3f}')

                # Early stopping
                if output['accuracy'][-1] > best_accuracy:
                    best_accuracy = output['accuracy'][-1]
                    th.save(model.state_dict(), './output/bert_model_ft.pt')
                    patience = max_patience
